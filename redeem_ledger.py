...
LEDGER_FOLDER = './trusted_ledger'

trusted_inputs = []
if os.path.exists(LEDGER_FOLDER):
    print('Trusted inputs found: loading...')
    for i in range(len(os.listdir(LEDGER_FOLDER))):
        with open(f'{LEDGER_FOLDER}/{i}.dat', 'r') as f:
    ...
    for i, input in enumerate(trusted_inputs):
        ...
        with open(f'{LEDGER_FOLDER}/{i}.dat', 'w') as f:
            f.write(serialized)
